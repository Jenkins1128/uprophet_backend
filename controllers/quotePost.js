const getQuotePost = async (req, res, db, jwt, refreshToken) => {
	...
	try {
		...
		const didLike = await trx('likes').select('users_id').where('users_id', id);
		console.log({ ...quotePost[0], likeCount: likeCount[0].likeCount, didLike: didLike.length ? true : false });
